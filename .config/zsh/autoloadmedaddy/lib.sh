...
grep_meta_val() {
  { grep --max-count=1 "^$1" || true; } | /bin/awk -F' : ' '{ print $2 }'
}
...
ocr_file() {
  ...
  case "$mimetype" in
    application/pdf)
      num_pages=$(pdfinfo "$if" | /bin/sed -n -E 's/^Pages:\s+([0-9]+)/\1/p')
...
get_all_isbns_from_archive() {
  ...
  /bin/find "$tmpdir" -type d -empty -delete
}
...
move_or_link_ebook_file_and_metadata() {
  ...
  while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    d["$(echo "${line%%:*}" | /bin/sed -e 's/[ \t]*$//' -e 's/ /_/g' -e 's/[^a-zA-Z0-9_]//g' -e 's/\(.*\)/\U\1/')"]="$(echo "${line#*: }" | /bin/sed -e 's/[\\/\*\?<>\|\x01-\x1F\x7F\x22\x24\x60]/_/g' | /bin/cut -c 1-100 )"
